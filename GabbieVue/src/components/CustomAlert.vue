<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-[3000] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white rounded-2xl w-[85%] max-w-xs shadow-2xl overflow-hidden animate-scale-in">
        ...
        <div class="border-t border-gray-100 p-4 flex gap-3">
          <button v-if="showCancel" @click="handleCancel" class="flex-1 py-3 rounded-xl font-bold text-sm text-gray-500 bg-gray-50 hover:bg-gray-100 transition-colors">
            Cancel
          </button>
          <button @click="handleConfirm" :class="['flex-1 py-3 rounded-xl font-bold text-sm transition-transform active:scale-95', btnClass]">
            {{ buttonText }}
          </button>
        </div>
      </div>
...
const show = (opts) => {
  ...
  showCancel.value = opts.showCancel || type.value === 'confirm'
  visible.value = true
...
const headerClass = computed(() => {
  const map = {
    ...
    confirm: 'bg-gradient-to-br from-violet-400 to-purple-600'
  }
...
const btnClass = computed(() => {
  const map = {
    ...
    confirm: 'bg-purple-50 text-purple-600 hover:bg-purple-100'
  }
  return map[type.value] || map.info
})

const buttonText = computed(() => {
  if (type.value === 'confirm') return 'Confirm'
