<template>
  <div class="relative h-full">
    ...
    <!-- Create Modal -->
    <Teleport to="body">
      <div v-if="showCreate" class="fixed inset-0 z-[2000] bg-white flex flex-col animate-slide-up">
        <!-- Modal Header -->
        <div class="px-4 py-3 pt-[max(env(safe-area-inset-top),20px)] border-b border-gray-100 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <!-- Form -->
        <div class="flex-1 overflow-y-auto p-4 pb-[calc(20px+env(safe-area-inset-bottom))]">
          <div class="space-y-6">
...
const searchLocation = async () => {
  ...
  try {
    ...
    if (data?.[0]) {
      ...
      alertRef.value.show({
        title: 'Found It!',
        message: 'Location Found: ' + form.location,
        type: 'success'
      })
    } else {
      alertRef.value.show({
    ...
    alertRef.value.show({
      title: 'Network Error',
      message: 'Could not connect to map service',
      type: 'error'
    })
  } finally {
...
const saveTrip = async () => {
  ...
  if (userStore.balance < 20) {
    const confirmed = await alertRef.value.show({
      title: 'Insufficient Coins',
      message: 'Need 20 coins. Go to Store?',
      type: 'confirm'
    })
  ...
  const confirmed = await alertRef.value.show({
    ...
    message: 'Publishing this entry will cost 20 Coins. Continue?',
    type: 'confirm'
  })
  ...
  if (userStore.spendCoins(20)) {
    ...
    await alertRef.value.show({
      title: 'Success!',
      message: 'Entry Saved! (-20 Coins)',
      type: 'success'
    })
  }
