  ...
  <div class="flex-1 flex flex-col h-full relative">
      ...
      <router-view v-slot="{ Component }">
        <transition name="fade-page" mode="out-in">
          <component 
            ...
            :class="{ 'pb-[calc(80px+env(safe-area-inset-bottom))]': isMainTab }"
          />
        </transition>
      </router-view>
    </main>
    ...
    <div v-if="showSettings" class="fixed inset-0 z-[2000] bg-black/40 backdrop-blur-sm flex justify-start" @click.self="showSettings = false">
        <div class="w-3/4 max-w-xs h-full bg-white shadow-2xl p-6 flex flex-col animate-slide-right">
            ...
            <div class="space-y-2">
                <button @click="openWebview('Privacy Policy', 'https://ipa.gabbiey.com/PrivacyTermsAndConditions.html')" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 text-gray-700">
                    <i class="ph-bold ph-shield-check text-xl"></i>
                    <span class="font-medium">Privacy Policy</span>
                </button>
                <button @click="openWebview('Terms of Service', 'https://ipa.gabbiey.com/MainstreamUserTerms.html')" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 text-gray-700">
